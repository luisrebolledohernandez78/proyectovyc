...
class WorkOrderAdmin(admin.ModelAdmin):
    ...
    def quote_pdf_link(self, obj):
        if hasattr(obj, "quote") and obj.quote:
            ...
            return format_html('<a class="button" href="{}" target="_blank">PDF Cotizacion</a>', url)
        return "-"
...
def cerrar_ot(modeladmin, request, queryset):
    ...
    for work_order in queryset:
        if work_order.status != work_order.DONE:
            work_order.status = work_order.DONE
            work_order.closed_at = timezone.now()
            work_order.save(update_fields=["status", "closed_at"])
            updated += 1
